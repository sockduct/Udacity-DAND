...
str(df, list.len=5)  # 113,937 observations of 81 variables
```
...
names(df)[names(df) == 'TradesNeverDelinquent (percentage)'] <-
					   'TradesNeverDelinquent.pct'
...
# df[, 'LoanStatus'] <- lapply(df[, 'LoanStatus'], function(x)
#					    ifelse(startsWith(x, 'Past Due'), 'Past Due', x))
...
# tdf <- df[df$LoanStatus %in% c('Past Due (1-15 days)',
...
# Already have "Not available" category for EmploymentStatus, consolidate NAs
# here:
...
# between duplicates - if TRUE, this is only difference:
all(sapply(dups.by.lk, function(x) all(x[1] == x)))
...
Using [Investopedia.com's Understanding the Five Cs of Credit?](\
...
df <- df[c('DebtToIncomeRatio', 'IncomeVerifiable', 'StatedMonthlyIncome',
           ...
           'RevolvingCreditBalance', 'Term', 'BorrowerAPR',
           'LoanOriginalAmount', 'ListingCategory.numeric', 'AmountDelinquent',
...
# Loan Original Amount in USD - included to show range of the loans
i <- 'LoanOriginalAmount'
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) + 
	geom_histogram(bins=35, color='blue', fill='orange') + 
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000, 17000), labels=comma) + 
	scale_x_continuous(breaks=c(1000, 5000, 10000, 15000, 20000, 25000, 30000,
		35000), labels=comma) + 
...
> Terms appear to be 1, 3, or 5 years.  Knowing this we can also examine loans
by term:

```{r echo=FALSE, warning=FALSE, Univariate_Plots2}
# Term in Months - included to show range of loan terms
i <- 'Term'
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) + 
	geom_histogram(bins=5, color='blue', fill='orange') + 
...
writeLines('Loan Amounts by Term:')
i <- 'LoanOriginalAmount'
j <- 'Term'

ggplot(aes_(x=as.name(j), y=as.name(i), group=as.name(j)), data=subset(df,
		!is.na(df[[i]]))) + 
	geom_boxplot(color='blue', outlier.shape=21) + 
	scale_y_continuous(breaks=seq(0, 35000, 5000), labels=comma) +
	scale_x_continuous(breaks=c(12, 36, 60)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5))
```
...
# Borrower APR - included to show range of APRs charged to borrowers
i <- 'BorrowerAPR'
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_histogram(bins=30, color='blue', fill='orange') +
...
with the rest in final payment or late.  This blog entry - [Understanding Loan
Statuses](https://www.orchardplatform.com/blog/understanding-loan-statuses/)
provides additional clarity on this terminology.
```{r echo=FALSE, warning=FALSE, Univariate_Plots4}
# Loan Status - included to show how loans fare overall
tdf <- data.frame(table(df$LoanStatus))
past.due <- sum(tdf[grep('Past Due', tdf$Var1), 2])
tcol1 <- factor(x=c('Completed', 'Current', 'FinalPaymentInProgress',
                    'Cancelled', 'Past Due', 'Defaulted', 'Chargedoff'),
...
# Must avoid the dreaded pie chart per reviewer!  :-)  But non-technical people
# really like them...
# Plot Pie Chart:
# ggplot(tdf, aes(x='', y=Count, fill=LoanStatus)) +
#  geom_bar(width=1, stat="identity") +
#  coord_polar("y", start=0) + scale_fill_brewer(palette='Accent', type='qual')
#  + theme(axis.title=element_blank(), axis.text=element_blank())
# writeLines('\n')

# Reviewer didn't like using color here because it doesn't encode additional
# information.  However, I believe it looks more "aesthetically pleasing".
# Right answer probably depends on the target audience...
# Plot Bar Chart:
# Change factor order for plot
tdf$LoanStatus <- ordered(tdf$LoanStatus, levels=c("Current", "Completed",
                          "Chargedoff", "Defaulted", "Past Due",
                          "FinalPaymentInProgress", "Cancelled"))
ggplot(tdf, aes(x=LoanStatus, y=Count, fill=LoanStatus)) +
	geom_bar(width=0.8, stat="identity") +
	scale_fill_brewer(palette='Accent', type='qual') +
	geom_text(aes(label=(paste0(round((Count/sum(Count) * 100), digits=2), '%')),
		vjust=-0.6)) +
	geom_text(aes(label=(paste0('(', Count, ')')), vjust=+1.6)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5),
		axis.text.x=element_text(angle=15)) +
	scale_y_continuous(breaks=seq(0, 60000, 10000), labels=comma)

# show.sum('LoanStatus')  # Not needed, shown in bar chart
# print(tdf[order(tdf$Count, decreasing=T), ])
```

> How are the loans used?  Examining the listing categories on a log scale we
see the following:

```{r echo=FALSE, warning=FALSE, Univariate_Plots4b}
tdf <- data.frame(Category=c('Not Available', 'Debt Consolidation',
		   'Home Improvement', 'Business', 'Personal Loan', 'Student Use', 'Auto',
		   ...
		   'Vacation', 'Wedding Loans'), Count=summary(df$ListingCategory.numeric))

ggplot(aes(x=Count, y=reorder(Category, Count)), data=tdf) +
	geom_point(color='blue', size=5) +
	scale_x_log10(breaks=c(50, 200, 800, 4000, 15000, 60000), labels=comma) +
	theme_bw() +
...
> Incomes span a huge range, but the average is close to $5,000/month.  If we
segment that by employment status though, we can see that part-time and the
unemployed groups are quite a bit below this average:
```{r echo=FALSE, warning=FALSE, Univariate_Plots5}
# About Borrowers:

# Stated Monthly Income in USD - included to show range of borrower income
i <- 'StatedMonthlyIncome'
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_histogram(bins=30, color='blue', fill='orange') +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000, 40000), labels=comma) +
...
writeLines('Income by Employment Status:')
j <- 'EmploymentStatus'
...
with most of them being so for over 2 years.  Again though, if we segment by
employment status we see that part time workers quite a bit lower than the
other groups.  This makes intuitive sense.  Interestingly, the "unemployed"
group, while lower still has a positive number.  So perhaps this represents
past employment for them?

```{r echo=FALSE, warning=FALSE, Univariate_Plots6}
# Employment Status Duration in Months - included to show range of borrower
# employment duration
i <- 'EmploymentStatusDuration'
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_histogram(bins=30, color='blue', fill='orange') +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000, 21000), labels=comma) +
	scale_x_sqrt(breaks=c(10, 80, 200, 400, 800)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5))
...
ggplot(aes_(x=as.name(j), y=as.name(i), group=as.name(j)), data=subset(df,
		!is.na(df[[i]]))) +
	geom_boxplot(color='blue', outlier.shape=21) +
	scale_y_sqrt(breaks=c(2, 18, 48, 96, 192, 384, 768)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5))
```

> Most borrowers are repaying at a rate of around $100 - $400/month though
there are some outliers.  Examing by employment status, we see that part-time,
retired, and not available have a median repayment amount near the bottom of
this range with many in their groups repayinging under $100/month:
```{r echo=FALSE, warning=FALSE, Univariate_Plots7}
# Monthly Loan Payment in USD - included to show range of borrower loan
# repayment amounts
i <- 'MonthlyLoanPayment'
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_histogram(bins=30, color='blue', fill='orange') +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000, 30000), labels=comma) +
	scale_x_continuous(breaks=seq(0, 2500, 500)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5))

show.sum(i)

writeLines('Loan Repayment by Employment Status:')
j <- 'EmploymentStatus'

ggplot(aes_(x=as.name(j), y=as.name(i), group=as.name(j)), data=subset(df,
		!is.na(df[[i]]))) +
	geom_boxplot(color='blue', outlier.shape=21) +
	scale_y_sqrt(breaks=c(10, 120, 400, 800, 1500, 2300), labels=comma) +
	theme(panel.grid.major=element_line(color='gray', size=0.5),
		axis.text.x=element_text(angle=15))
```

> Most borrowers have significant revolving credit balances, from $3,000 - 
$20,000 with some in excess of a million.  Examing by employment status shows
the part-time group has the lowest interquartile-range of revolving debt.  The
highest levels of debt seem to be limited to the full-time, self-employed, and
employed groups:

```{r echo=FALSE, warning=FALSE, Univariate_Plots8}
# Revolving Credit Balance in USD - included to show borrower revolving (e.g.,
# typically credit card) debt
i <- 'RevolvingCreditBalance'
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_histogram(bins=30, color='blue', fill='orange') +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000, 16000), labels=comma) +
	scale_x_log10(breaks=c(1, 10, 100, 1000, 10000, 100000, 1000000),
		labels=comma) +
	theme(panel.grid.major=element_line(color='gray', size=0.5))
...
writeLines('Revolving Credit Balance by Employment Status:')
j <- 'EmploymentStatus'
...
is good, .36 - .49 is marginal, and .5 or higher is problematic.  The
unemployed group also stands out for its fairly evenly distributed ratios all
the way from good to truly awful:

```{r echo=FALSE, warning=FALSE, Univariate_Plots9}
# Debt to Income Ratio - included to show borrower ratio (compared to what's
# "good")
i <- 'DebtToIncomeRatio'
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_histogram(bins=35, color='blue', fill='orange') +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000, 30000), labels=comma) +
	scale_x_sqrt(breaks=c(.05, .65, 2, 4, 6.5, 10)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5))

show.sum(i)

writeLines('Debt-to-Income Ratio by Employment Status:')
j <- 'EmploymentStatus'

ggplot(aes_(x=as.name(j), y=as.name(i), group=as.name(j)), data=subset(df,
		!is.na(df[[i]]))) +
	geom_boxplot(color='blue', outlier.shape=21) +
	scale_y_sqrt(breaks=c(.03, .35, 1, 2, 4, 6.5, 10)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5),
		axis.text.x=element_text(angle=15))
```

> Credit scores range from a little over 400 to a little under 900.  We see the
Not available group not only has the worst interquartile-range, but is also the
only group with scores of 0.  To put the scores in perspective, here's how
CreditSesame.com ranks them:
...
* For credit score there was an upper and lower range.  To keep things simple\
and conservative, only the lower range was selected.
* ProsperRating.alpha and CreditGrade are virtually the same - the reason they\
are both included is that CreditGrade applies only to loans before 2009, and\
ProsperRating.alpha applies to loans after July, 2009.
```{r echo=FALSE, warning=FALSE, Univariate_Plots10}
# Credit Score Range Lower - included to show borrower credit score (compared to
# what's "good")
i <- 'CreditScoreRangeLower'
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_histogram(bins=30, color='blue', fill='orange') +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000, 33000), labels=comma) +
...
writeLines('Credit Score by Employment Status:')
j <- 'EmploymentStatus'

ggplot(aes_(x=as.name(j), y=as.name(i), group=as.name(j)), data=subset(df,
		!is.na(df[[i]]))) +
	geom_boxplot(color='blue', outlier.shape=21) +
	scale_y_continuous(breaks=seq(0, 880, 120)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5),
		axis.text.x=element_text(angle=15))
```
...
# Investment From Friends Count - included to show if friends invest
i <- 'InvestmentFromFriendsCount'
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_histogram(bins=34, color='blue', fill='orange') +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000, 100000), labels=comma) +
...
# Recommendations - included to show if recommendations play a significant role
i <- 'Recommendations'
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_histogram(bins=40, color='blue', fill='orange') +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000, 100000), labels=comma) +
...
Of note - the Not available group is significantly worse than the others:
```{r echo=FALSE, warning=FALSE, Univariate_Plots12}
# Problem Children:
# Delinquencies Last 7 Years - included to show borrower delinquency count
i <- 'DelinquenciesLast7Years'
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_histogram(bins=30, color='blue', fill='orange') +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000, 75000), labels=comma) 
	scale_x_sqrt(breaks=c(0, 1, 8, 20, 40, 67, 100)) 
	theme(panel.grid.major=element_line(color='gray', size=0.5))
...
million.  Breaking this down by employment status doesn't seem to reveal
additional insight:

```{r echo=FALSE, warning=FALSE, Univariate_Plots13}
# Amount Delinquent - included to show borrower delinquency amount
i <- 'AmountDelinquent'
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_histogram(bins=30, color='blue', fill='orange') +
...
writeLines('Amount Delinquent by Employment Status:')
j <- 'EmploymentStatus'

ggplot(aes_(x=as.name(j), y=as.name(i), group=as.name(j)), data=subset(df,
		!is.na(df[[i]]))) +
	geom_boxplot(color='blue', outlier.shape=21) +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000, 100000, 500000),
		labels=comma) +
	theme(panel.grid.major=element_line(color='gray', size=0.5))
```

> Most borrowers have no public records on file (e.g., bankruptcies, tax liens,
civil judgements).  However, some have ten or more.  Breaking this down by
employment status, we see that Retired, Other, and Not Available are
significantly worse than other groups:

```{r echo=FALSE, warning=FALSE, Univariate_Plots14}
# Public Records Last 10 Years - included to show number of public records
# borrower has
i <- 'PublicRecordsLast10Years'
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_histogram(bins=39, color='blue', fill='orange') +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000, 90000), labels=comma) +
	scale_x_continuous(breaks=seq(0, 40, 5)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5))

show.sum(i)

writeLines('Public Records in Last 10 Years by Employment Status:')
j <- 'EmploymentStatus'
...
# cor.tbl2 <- cor(dfo[, c(4, 7:12, 15, 20, 24:25, 27:45, 48:60, 63:76)],
...
x <- cdf[1, 2]
y <- cdf[1, 3]
ggplot(aes_(x=as.name(x), y=as.name(y)), data=df) +
	geom_jitter(color='blue', alpha=.1, na.rm=T) +
	scale_x_continuous(breaks=c(1000, 5000, 10000, 15000, 20000, 25000, 30000,
		35000), labels=comma) +
...
y <- cdf[2, 3]
ggplot(aes_(x=as.name(x), y=as.name(y)), data=df) +
	geom_point(color='blue', alpha=.05, na.rm=T) +
	scale_x_continuous() +
	scale_y_continuous() +
	theme(panel.grid.major=element_line(color='gray', size=0.5)) +
	geom_smooth(method='lm', color='orange', na.rm=T) +
...
# Note:
# cor(df[df$CreditScoreRangeLower > 0, ]$CreditScoreRangeLower,
#     df[df$CreditScoreRangeLower > 0, ]$BorrowerAPR, use='complete.obs')
# --> -0.4561199, slightly better correlation (before was -0.4292891)

# Plot - CreditScoreRangeLower/BorrowerAPR, -.46
x <- cdf[3, 2]
y <- cdf[3, 3]
ggplot(aes_(x=as.name(x), y=as.name(y)), data=df[df$CreditScoreRangeLower > 0,
		 ]) +
	 geom_point(color='blue', alpha=.05, na.rm=T) +
	 scale_x_continuous() +
	 scale_y_continuous() +
...
# Note:
# cor(df[df$CreditScoreRangeLower > 0, ]$CreditScoreRangeLower,
#    df[df$CreditScoreRangeLower > 0, ]$LoanOriginalAmount, use='complete.obs')
# --> 0.3527187, slightly better correlation (before was 0.3414884)
...
y <- cdf[6, 3]
ggplot(aes_(x=as.name(x), y=as.name(y)), data=df) +
	geom_point(color='blue', alpha=.8, na.rm=T) +
	scale_x_sqrt(breaks=c(1, 4, 10, 20, 30, 40)) +
	scale_y_sqrt(breaks=c(1, 4, 10, 20, 35)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5)) +
	geom_smooth(method='lm', color='orange', na.rm=T) +
	geom_smooth(color='red', na.rm=T)
```
...
> Borrower APR faceted by Employment Status - while no major differences are
apparent, the Not available group has the highest APRs, and the unemployed
have a pronounced spike of APRs at the higher end (around .35):

```{r echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots6b}
i <- 'BorrowerAPR'
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_histogram(bins=30, color='blue', fill='orange') +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000), labels=comma) +
...
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_freqpoly(bins=45, aes(color=IncomeVerifiable)) +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000), labels=comma) +
	scale_x_continuous(breaks=seq(0, .5, .1)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5))
```
...
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_freqpoly(bins=45, aes(color=IsBorrowerHomeowner)) +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000), labels=comma) +
	scale_x_continuous(breaks=seq(0, .5, .1)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5))
```
...
# Right answer probably depends on the target audience...
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]) &
		!is.na(df$CreditGrade))) +
	geom_freqpoly(bins=45, aes(color=CreditGrade)) +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000), labels=comma) +
	scale_x_continuous(breaks=seq(0, .5, .1)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5)) +
	facet_wrap(~CreditGrade) +
	geom_vline(data=filter(df, CreditGrade == 'AA'),
	  aes(xintercept=res[['AA']])) +
  geom_vline(data=filter(df, CreditGrade == 'A'),
    aes(xintercept=res[['A']])) +
  geom_vline(data=filter(df, CreditGrade == 'B'),
    aes(xintercept=res[['B']])) +
  geom_vline(data=filter(df, CreditGrade == 'C'),
    aes(xintercept=res[['C']])) +
  geom_vline(data=filter(df, CreditGrade == 'D'),
    aes(xintercept=res[['D']])) +
  geom_vline(data=filter(df, CreditGrade == 'E'),
    aes(xintercept=res[['E']])) +
  geom_vline(data=filter(df, CreditGrade == 'HR'),
		aes(xintercept=res[['HR']])) +
	geom_vline(data=filter(df, CreditGrade == 'NC'), aes(xintercept=res[['NC']]))
...
# Right answer probably depends on the target audience...
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]) &
		!is.na(df$ProsperRating.alpha))) +
	geom_freqpoly(bins=45, aes(color=ProsperRating.alpha)) +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000), labels=comma) +
	scale_x_continuous(breaks=seq(0, .5, .1)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5)) +
	facet_wrap(~ProsperRating.alpha) +
	geom_vline(data=filter(df, ProsperRating.alpha == 'AA'),
		aes(xintercept=res[['AA']])) +
	geom_vline(data=filter(df, ProsperRating.alpha == 'A'),
		aes(xintercept=res[['A']])) +
	geom_vline(data=filter(df, ProsperRating.alpha == 'B'),
		aes(xintercept=res[['B']])) +
	geom_vline(data=filter(df, ProsperRating.alpha == 'C'),
		aes(xintercept=res[['C']])) +
	geom_vline(data=filter(df, ProsperRating.alpha == 'D'),
		aes(xintercept=res[['D']])) +
	geom_vline(data=filter(df, ProsperRating.alpha == 'E'),
		aes(xintercept=res[['E']])) +
	geom_vline(data=filter(df, ProsperRating.alpha == 'HR'),
		aes(xintercept=res[['HR']]))
...
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]))) +
	geom_histogram(bins=30, color='blue', fill='orange') +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 10000, 50000), labels=comma) +
	scale_x_sqrt(breaks=c(2500, 75000, 250000), labels=comma) +
	theme(panel.grid.major=element_line(color='gray', size=0.5)) +
	facet_wrap(~LoanStatus)
```
...
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]) &
		!is.na(df$CreditGrade))) +
	geom_freqpoly(bins=45, aes(color=CreditGrade)) +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 4000), labels=comma) +
	scale_x_continuous(breaks=seq(0, 880, 160)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5)) +
	facet_wrap(~CreditGrade)
```
...
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]) &
		!is.na(df$ProsperRating.alpha))) +
	geom_freqpoly(bins=15, aes(color=ProsperRating.alpha)) +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 4000), labels=comma) +
	scale_x_continuous(breaks=seq(600, 880, 80)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5)) +
	facet_wrap(~ProsperRating.alpha) +
	geom_vline(data=filter(df, ProsperRating.alpha == 'AA'),
		aes(xintercept=res[['AA']])) +
...
print(with(df, by(CreditScoreRangeLower, ProsperRating.alpha, summary,
	na.rm=T)))
```
...
i <- 'CreditScoreRangeLower'
j <- 'BorrowerAPR'
ggplot(aes_(x=as.name(i), y=as.name(j)), data=subset(df, !is.na(df[[i]]) &
		!is.na(df$ProsperRating.alpha) & !is.na(df[[j]]))) +
	geom_point(aes(color=Term)) +
	scale_x_continuous(breaks=seq(600, 880, 80)) +
	scale_y_continuous(breaks=seq(0.05, .45, .1)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5)) +
	facet_wrap(~ProsperRating.alpha) +
	geom_smooth(method='lm', color='orange', na.rm=T) +
	geom_smooth(color='red', na.rm=T)
...
> Borrower credit score vs. APR faceted by Prosper rating colored by employment
status.  The dominant employment groups are employed and full time.  While a
little subtle, as the rating worsens, we can see the plots get more colorful.
This is a result of including more of the groups which appear to have a higher
...
ggplot(aes_(x=as.name(i), y=as.name(j)), data=subset(df, !is.na(df[[i]]) &
		!is.na(df$ProsperRating.alpha) & !is.na(df[[j]]))) +
	geom_point(aes(color=IncomeVerifiable)) +
	scale_x_log10(labels=comma) +
	scale_y_continuous(breaks=seq(0.05, .45, .1)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5)) +
	facet_wrap(~ProsperRating.alpha, scales='free_x') +
	geom_smooth(method='lm', color='orange', na.rm=T) +
	geom_smooth(color='red', na.rm=T)
...
ggplot(aes_(x=as.name(i), y=as.name(j)), data=subset(df, !is.na(df[[i]]) &
	...
	scale_x_sqrt(breaks=c(.35, 2, 5, 10), labels=comma) +
	scale_y_continuous(breaks=seq(0.05, .45, .1)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5)) +
	facet_wrap(~ProsperRating.alpha)
...
print(with(df, by(RevolvingCreditBalance, ProsperRating.alpha, summary,
	na.rm=T)))
```
...
ggplot(aes_(x=as.name(i)), data=subset(df, !is.na(df[[i]]) &
		!is.na(df$ProsperRating.alpha))) +
	geom_freqpoly(bins=15, aes(color=ProsperRating.alpha)) +
	scale_y_log10(breaks=c(1, 10, 100, 1000, 5000), labels=comma) +
	scale_x_continuous(breaks=seq(600, 880, 80)) +
	theme(panel.grid.major=element_line(color='gray', size=0.5),
  ...
  labs(caption='[Vertical black bars show average credit score]') +
  labs(subtitle='--Post July, 2009--') +
  labs(title='Borrower Credit Score faceted by Prosper Rating') +
...
ggplot(aes_(x=as.name(i), y=as.name(j)), data=subset(df, !is.na(df[[i]]) &
	...
	ylab('Loan APR') +
	labs(caption=
		'[Orange line shows linear trend, Red line shows nonlinear trend]') +
	labs(subtitle=paste0('Colored by Loan Term in Months, Faceted by Prosper ',
		   'Rating (Post July, 2009)')) +
  labs(title='Loan APR for Borrower Credit Scores')
```
...
ggplot(aes_(x=as.name(i), y=as.name(j)), data=subset(df, !is.na(df[[i]]) &
	...
	geom_smooth(method='lm', color='green', na.rm=T) +
	geom_smooth(color='yellow', na.rm=T) +
	xlab('Loan Amount') +
	ylab('Borrower Monthly Income') +
	labs(caption=
		'[Green line shows linear trend, Yellow line shows nonlinear trend]') +
	labs(subtitle=
		'Colored by Loan Status, Faceted by Prosper Rating (Post July, 2009)') +
	labs(title='Borrower Monthly Income for Loan Amount') +
