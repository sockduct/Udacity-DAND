...
# Already have "Not available" category for EmploymentStatus, consolidate NAs here:
...
ggplot(aes_(x=as.name(j), y=as.name(i), group=as.name(j)), data=subset(df, !is.na(df[[i]]))) + geom_boxplot(color='blue', outlier.shape=21) + scale_y_continuous(breaks=seq(0, 35000, 5000), labels=comma) + scale_x_continuous(breaks=c(12, 36, 60)) + theme(panel.grid.major=element_line(color='gray', size=0.5))
```
...
# Must avoid the dreaded pie chart per reviewer!  :-)  But non-technical people really like them...
# Plot Pie Chart:
# ggplot(tdf, aes(x='', y=Count, fill=LoanStatus)) + geom_bar(width=1, stat="identity") + coord_polar("y", start=0) + scale_fill_brewer(palette='Accent', type='qual') + theme(axis.title=element_blank(), axis.text=element_blank())
# writeLines('\n')
...
tdf$LoanStatus <- ordered(tdf$LoanStatus, levels=c("Current", "Completed",
                          ...
                          "FinalPaymentInProgress", "Cancelled"))
ggplot(tdf, aes(x=LoanStatus, y=Count, fill=LoanStatus)) + geom_bar(width=0.8, stat="identity") + scale_fill_brewer(palette='Accent', type='qual') + geom_text(aes(label=(paste0(round((Count/sum(Count) * 100), digits=2), '%')), vjust=-0.6)) + geom_text(aes(label=(paste0('(', Count, ')')), vjust=+1.6)) + theme(panel.grid.major=element_line(color='gray', size=0.5), axis.text.x=element_text(angle=15)) + scale_y_continuous(breaks=seq(0, 60000, 10000), labels=comma)
...
tdf <- data.frame(Category=c('Not Available', 'Debt Consolidation', 'Home Improvement', 'Business', 'Personal Loan', 'Student Use', 'Auto', 'Other', 'Baby & Adoption', 'Boat', 'Cosmetic Procedure', 'Engagement Ring', 'Green Loans', 'Household Expenses', 'Large Purchases', 'Medical/Dental', 'Motorcycle', 'RV', 'Taxes', 'Vacation', 'Wedding Loans'), Count=summary(df$ListingCategory.numeric))

ggplot(aes(x=Count, y=reorder(Category, Count)), data=tdf) + geom_point(color='blue', size=5) + scale_x_log10(breaks=c(50, 200, 800, 4000, 15000, 60000), labels=comma) + theme_bw() + theme(panel.grid.major.x=element_line(color='gray', size=0.5), panel.grid.major.y=element_line(colour="gray", linetype="dashed")) + ylab('Listing Category')
```
...
ggplot(aes_(x=as.name(j), y=as.name(i), group=as.name(j)), data=subset(df, !is.na(df[[i]]))) + geom_boxplot(color='blue', outlier.shape=21) + scale_y_log10(breaks=c(1, 10, 100, 1000, 10000, 100000, 1000000), labels=comma) + theme(panel.grid.major=element_line(color='gray', size=0.5), axis.text.x=element_text(angle=15))
```
...
# Employment Status Duration in Months - included to show range of borrower employment duration
...
ggplot(aes_(x=as.name(j), y=as.name(i), group=as.name(j)), data=subset(df, !is.na(df[[i]]))) + geom_boxplot(color='blue', outlier.shape=21) + scale_y_sqrt(breaks=c(2, 18, 48, 96, 192, 384, 768)) + theme(panel.grid.major=element_line(color='gray', size=0.5))
```
...
# Monthly Loan Payment in USD - included to show range of borrower loan repayment amounts
...
ggplot(aes_(x=as.name(j), y=as.name(i), group=as.name(j)), data=subset(df, !is.na(df[[i]]))) + geom_boxplot(color='blue', outlier.shape=21) + scale_y_sqrt(breaks=c(10, 120, 400, 800, 1500, 2300), labels=comma) + theme(panel.grid.major=element_line(color='gray', size=0.5), axis.text.x=element_text(angle=15))
```
...
# Revolving Credit Balance in USD - included to show borrower revolving (e.g., typically credit card) debt
...
# Debt to Income Ratio - included to show borrower ratio (compared to what's "good")
i <- 'DebtToIncomeRatio'
...
ggplot(aes_(x=as.name(j), y=as.name(i), group=as.name(j)), data=subset(df, !is.na(df[[i]]))) + geom_boxplot(color='blue', outlier.shape=21) + scale_y_sqrt(breaks=c(.03, .35, 1, 2, 4, 6.5, 10)) + theme(panel.grid.major=element_line(color='gray', size=0.5), axis.text.x=element_text(angle=15))
```
...
# Credit Score Range Lower - included to show borrower credit score (compared to what's "good")
...
ggplot(aes_(x=as.name(j), y=as.name(i), group=as.name(j)), data=subset(df, !is.na(df[[i]]))) + geom_boxplot(color='blue', outlier.shape=21) + scale_y_continuous(breaks=seq(0, 880, 120)) + theme(panel.grid.major=element_line(color='gray', size=0.5), axis.text.x=element_text(angle=15))
```
...
ggplot(aes_(x=as.name(j), y=as.name(i), group=as.name(j)), data=subset(df, !is.na(df[[i]]))) + geom_boxplot(color='blue', outlier.shape=21) + scale_y_continuous() + theme(panel.grid.major=element_line(color='gray', size=0.5), axis.text.x=element_text(angle=15))
```
...
ggplot(aes_(x=as.name(j), y=as.name(i), group=as.name(j)), data=subset(df, !is.na(df[[i]]))) + geom_boxplot(color='blue', outlier.shape=21) + scale_y_log10(breaks=c(1, 10, 100, 1000, 10000, 100000, 500000), labels=comma) + theme(panel.grid.major=element_line(color='gray', size=0.5))
```
...
# Public Records Last 10 Years - included to show number of public records borrower has
...
i <- 'CreditScoreRangeLower'
j <- 'BorrowerAPR'
ggplot(aes_(x=as.name(i), y=as.name(j)), data=subset(df, !is.na(df[[i]]) & !is.na(df$ProsperRating.alpha) & !is.na(df[[j]]))) + geom_point(aes(color=EmploymentStatus)) + scale_x_continuous(breaks=seq(600, 880, 80)) + scale_y_continuous(breaks=seq(0.05, .45, .1)) + theme(panel.grid.major=element_line(color='gray', size=0.5)) + facet_wrap(~ProsperRating.alpha) + geom_smooth(method='lm', color='orange', na.rm=T) + geom_smooth(color='red', na.rm=T) + scale_color_brewer(palette='Paired', type='qual')
```
